...
class Feature(BaseModel):
    ...
    mandatory_qualifiers_raw: List[str] = Field(
        default_factory=list,
        description="Qualifiers required with the key",
    )
...
def _parse_section_lines(header: str, lines: List[str]) -> str | List[str]:
    ...
    as_string = {"feature key", "definition", "comment", "organism scope", "molecule scope", "example"}
...
def extract_features_from_html(html: str) -> List[Feature]:
    ...
    for block in blocks:
        ...
        for i, line in enumerate(lines):
            if line.strip() and not re.match(r"^\s", line):
                ...
                if header in COL_HEADERS:
                    ...
                    raise ValueError(f"Unrecognized header '{header}' in block.")
        ...
        feature = Feature(
            ...
            mandatory_qualifiers_raw=section_map.get("mandatory qualifiers", []),  # type: ignore
    ...
    for f in features:
        for m in f.mandatory_qualifiers_raw:
            if not m.startswith("/"):
                print(f.feature_key)
                print(m)
