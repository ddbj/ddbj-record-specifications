...
class GeaSubmission(BaseModel):
    """GEA の IDF の、MAGE-TAB の版と SDRF のファイル名、GEA が Comment[...] に書く項目。"""

    magetab_version: str | None = Field(None, examples=["1.1"])
    sdrf_file: str | None = Field(None, examples=["E-GEAD-1005.sdrf.txt"])
...
class ExperimentalFactor(BaseModel):
    """MAGE-TAB の IDF の Experimental Factor Name / Type の 1 組。

    値は experiments[].pool.members[].factor_values に置く。
    """
...
class Protocol(BaseModel):
    """MAGE-TAB の IDF の Protocol Name / Type / Description の 1 組 (CIBEX の Protocol も)。

    name は、各オブジェクトの protocol_refs から指される。
    """
...
class Project(BaseModel):
    ...
    # SRA / JGA の研究の種類 ("WGS"、"Case-Control" など) と、MAGE-TAB の Experimental Design。
...
class Platform(BaseModel):
    ...
    # MAGE-TAB の Array Design REF。db は "array_design"。
...
class Extract(BaseModel):
    """MAGE-TAB の Extract。Source から取り出した材料。

    comments は Comment[x] の列 (name が x)。
    """

    name: str | None = None
    material_type: str | None = Field(None, examples=["total RNA"])
    # この材料を作るのに使った protocol。MAGE-TAB でこのノードの前に並ぶ Protocol REF。
...
class FactorValue(BaseModel):
    """MAGE-TAB の Factor Value[name] の列と、その後の Unit[unit_type] の列。"""
...
class PoolMember(BaseModel):
    ...
    MAGE-TAB では、Assay に入る Labeled Extract (無ければ Extract) の 1 つ。member_name が
    Labeled Extract Name、sample が元の Source、extract が途中の Extract。
    member_name は run / analysis の data_blocks[].member_name から指される。
    """
    ...
    # MAGE-TAB の Labeled Extract の Label と、その前に並ぶ Protocol REF。
...
class Experiment(BaseModel):
    ...
    # MAGE-TAB の Assay の Technology Type、前に並ぶ Protocol REF、Comment[x] の列。
...
class File(BaseModel):
    ...
    # MAGE-TAB では、ファイルの列の名前 ("Array Data File" など)。
