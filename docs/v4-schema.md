...
- JSON は I-JSON（RFC 7493）のもの。UTF-8 で書き、BOM を付けない。`NaN`、`Infinity`、対になっていないサロゲート（`\ud800`）、同じ key の繰り返しは使わない。数は、IEEE 754 の倍精度より大きいものや精度の高いものを使わない（RFC 7493 §2.2）。読む側によって値が変わるため
  - 整数は ±(2^53 − 1) まで。`9007199254740992` は使わない
  - 小数や指数のある数は、有効数字（前後の 0 を除いた桁）が 17 桁まで。倍精度の最も短い表記でなくてもよい。`0.1`、`1.10`、`1E+2`、`-0.0`、`0E0`、`0.10000000000000001`、`4.9e-324`、`1.7976931348623157e308` は使える。`0.100000000000000000001`（21 桁）は使わない
  - 倍精度の範囲を外れる数は使わない。大きすぎて無限大になるもの（`1e400`）も、0 でないのに小さすぎて 0 になるもの（`1e-400`）も
  - 数は 1 つの値として読み、書いた形（`1.10` と `1.1`）は保たない
- 改行は LF か CRLF。1 行は 1 MiB まで（配列を FASTA に出すので、1 つのオブジェクトはこれに収まる）
...
- JSON Lines の各行が、その list の要素のモデルに型を読み替えずに合う。空のファイル、空の行、行の途中の CR、I-JSON でないもの（BOM、`NaN`、上の範囲を外れる数、対になっていないサロゲート、同じ key の繰り返し）は問題。`record.json` も同じ約束で読む
