...
- **BioProject / 関連する研究への参照**: IDF の `Comment[BioProject]`（`part_of`）と `Comment[Related study]`（`related_to`）は `relations` に置く。起点は `{"type": "investigation", "accession": ...}`（accession を振る前の版は `alias`）
...
- **他の DB と同じ意味の項目**: IDF の Person と Public Release Date（過去の版の Comment[Public Release Date] も）、ADF の Comment[Public Release Date] は、`investigation` / `array_design` ではなく `submission.submitters` / `submission.hold_date` に置く。IDF の Comment[Last Update Date] は `investigation.legacy`（[v3-gea.md](./v3-gea.md#他の-db-と同じ意味の欄)）
