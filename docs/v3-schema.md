...
- GEA の SDRF の行は record に持たず、オブジェクトと relations から作り直す。作り直した行が元の行と違うもの (順序、全く同じ行の繰り返し、作り直すと増える行の組み合わせ) は、`submission.gea.sdrf_row_order` で元の行に戻す。作り直し方は GEA の対応表 (`gea.yml`) の冒頭にある
